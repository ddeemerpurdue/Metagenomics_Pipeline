...
def blast_bin_files(query, format, log):
    '''
    Description
    '''
    ...
    with open(log, 'w') as lg:
        ...
        if blast:
            ...
            download_command = f"blastn -query {query} -subject {full_reference} -outfmt {str(format)} -out {output}"
            try:
                # Execute the command
                ...
                if retcode > 0:  # Any error, let's catch it
                    ...
                    lg.write(f"Process was terminated by signal {retcode}\n")
...
def blast_nobin_files(query, format, log):
    '''
    Description
    '''
    now = time.localtime(time.time())
    ...
    with open(log, 'w') as lg:
        ...
        for file in os.listdir(reference_path):
            ...
            if blast:
                make_directory_command = f"mkdir -p {output.split('/blastn')[0]}"
                os.system(make_directory_command)
                download_command = f"blastn -query {query} -subject {full_reference} -outfmt {str(format)} -out {output} -max_target_seqs 3"
                try:
                    # Execute the command
                    retcode = call(download_command, shell=True)
                    if retcode > 0:  # Any error, let's catch it
                        ...
                        print("Process was terminated by signal",
                              retcode, file=sys.stderr)
                        lg.write(
                            f"Process was terminated by signal {retcode}\n")
                        return 1  # If return 1, we won't write the token
                        # output file, halting the pipeline.
                    else:
                        pass
